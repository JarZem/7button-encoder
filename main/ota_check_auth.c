...
{
    if (input == NULL || strlen(input) != input_len) return ESP_ERR_INVALID_ARG;
    char padded[64];
    ...
    for (size_t i = 0; i < padded_len; ++i) {
        ...
        else if (!isalnum((unsigned char)padded[i])) return ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t derive_session_key(const ota_secure_state_nvs_t *ctx, uint8_t out[32])
{
    ...
    memcpy(material + pos, device_id, idlen); pos += idlen;
    put_u64_be(material + pos, ctx->counter); pos += 8;
    memcpy(material + pos, ctx->random, sizeof(ctx->random)); pos += sizeof(ctx->random);
...
static esp_err_t derive_subkey(const uint8_t session_key[32], const char *domain, uint8_t out[32])
{
...
static bool version_is_newer(const char *offered, const char *running)
{
    ...
    while (*a || *b) {
        ...
        if (end_a == a || end_b == b) { numeric = false; break; }
        if (va != vb) return va > vb;
        a = (*end_a == '.') ? end_a + 1 : end_a;
        b = (*end_b == '.') ? end_b + 1 : end_b;
        if ((*a && !isdigit((unsigned char)*a)) || (*b && !isdigit((unsigned char)*b))) { numeric = false; break; }
    }
...
static esp_err_t persist_active_grant(const uint8_t random[OTA_CHECK_RANDOM_LEN])
{
    ...
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(OTA_CHECK_NAMESPACE, NVS_READWRITE, &h), TAG, "open grant NVS failed");
...
static esp_err_t load_active_grant(uint8_t random[OTA_CHECK_RANDOM_LEN])
{
    ...
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(OTA_CHECK_NAMESPACE, NVS_READONLY, &h), TAG, "open grant NVS failed");
...
esp_err_t ota_check_auth_prepare_request(const char *payload, size_t payload_len,
                                         ...
                                         size_t *out_len)
{
    if (payload == NULL || out_request == NULL || out_len == NULL || payload_len == 0) return ESP_ERR_INVALID_ARG;
    if (payload_len >= 128) return ESP_ERR_INVALID_SIZE;
    ...
    ESP_RETURN_ON_ERROR(base64url_decode_exact(random_b64, OTA_CHECK_RANDOM_B64_LEN, grant_random, sizeof(grant_random)), TAG, "CHECK random invalid");
    ESP_RETURN_ON_ERROR(base64url_decode_exact(mac_b64, OTA_CHECK_MAC_B64_LEN, received_mac, sizeof(received_mac)), TAG, "CHECK MAC invalid");

    ota_secure_state_nvs_t ctx;
    ESP_RETURN_ON_ERROR(load_persisted_context(&ctx), TAG, "provisioning context missing");
    uint8_t session_key[32], check_key[32], token_key[32];
    ...
    size_t pos = 0;
    memcpy(canonical + pos, CHECK_MAC_DOMAIN, strlen(CHECK_MAC_DOMAIN)); pos += strlen(CHECK_MAC_DOMAIN);
    memcpy(canonical + pos, version, strlen(version)); pos += strlen(version);
    canonical[pos++] = '|';
    memcpy(canonical + pos, code, strlen(code)); pos += strlen(code);
    canonical[pos++] = '|';
    memcpy(canonical + pos, grant_random, sizeof(grant_random)); pos += sizeof(grant_random);
    uint8_t mac[32];
    ESP_RETURN_ON_ERROR(hmac_sha256(check_key, sizeof(check_key), canonical, pos, mac), TAG, "CHECK HMAC failed");
    ...
    ESP_RETURN_ON_ERROR(device_identity_get_device_id(device_id), TAG, "device id unavailable");
    pos = 0;
    memcpy(canonical + pos, TOKEN_DOMAIN, strlen(TOKEN_DOMAIN)); pos += strlen(TOKEN_DOMAIN);
    memcpy(canonical + pos, device_id, strlen(device_id)); pos += strlen(device_id);
    canonical[pos++] = '|';
    memcpy(canonical + pos, version, strlen(version)); pos += strlen(version);
    canonical[pos++] = '|';
    memcpy(canonical + pos, code, strlen(code)); pos += strlen(code);
    ...
    ESP_RETURN_ON_ERROR(hmac_sha256(token_key, sizeof(token_key), canonical, pos, token_digest), TAG, "token HMAC failed");
    char token[OTA_CONFIG_MAX_TOKEN_LEN + 1];
    ESP_RETURN_ON_ERROR(base64url_encode(token_digest, OTA_CHECK_TOKEN_RAW_LEN, token, sizeof(token)), TAG, "token encode failed");
    if (strlen(token) != OTA_CHECK_TOKEN_B64_LEN) return ESP_ERR_INVALID_SIZE;

    ota_secure_provisioning_t provision;
    ESP_RETURN_ON_ERROR(ota_secure_session_load_provisioning(&provision), TAG, "stored provisioning unavailable");
    ...
    if (!storage_save_ota_config(&config)) return ESP_FAIL;
    ...
    if (n <= 0 || (size_t)n >= out_size) return ESP_ERR_INVALID_SIZE;
    *out_len = (size_t)n;
...
esp_err_t ota_check_auth_build_completion(char out[OTA_CHECK_COMPLETION_MAX_LEN])
{
    ...
    ESP_RETURN_ON_ERROR(base64url_encode(random, sizeof(random), random_b64, sizeof(random_b64)), TAG, "completion random encode failed");
