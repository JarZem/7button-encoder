...
{
    ESP_LOGI(TAG,
             "STATE reason=%s switches=0x%02x rs232=%s light=%s brightness=%u%% white_temperature=%uK",
             reason,
             s_state.switches,
...
static void set_switch(uint8_t switch_id, bool enabled, const char *reason)
{
    ...
    set_switch_mask(enabled ? (uint8_t)(s_state.switches | bit)
                            : (uint8_t)(s_state.switches & (uint8_t)~bit), reason);
}
...
static void set_rs232(bool enabled, const char *reason)
{
...
static void handle_button_press(uint8_t button_id)
{
    if (button_id >= 1 && button_id <= DEVICE_SWITCH_COUNT) {
        set_switch(button_id, ((s_state.switches >> (button_id - 1U)) & 1U) == 0, "button");
    } else if (button_id == 7) {
        if (any_switch_on()) all_off("button7_all_off"); else restore_last("button7_restore");
    } else if (button_id == 8) {
...
static void handle_encoder_rotation(int32_t direction, TickType_t tick)
{
    if (s_encoder_mode == ENCODER_MODE_BRIGHTNESS) {
        ...
        set_brightness(clamp_u8((int32_t)s_state.brightness + direction * step,
                                DEVICE_BRIGHTNESS_MIN, DEVICE_BRIGHTNESS_MAX),
                       "encoder_brightness");
    } else {
        const int32_t step = accelerated_step(tick, 100, 200, 300, 500);
        set_white_temperature(clamp_u16((int32_t)s_state.white_temperature + direction * step,
                                        DEVICE_WHITE_TEMP_MIN, DEVICE_WHITE_TEMP_MAX),
                              "encoder_white_temperature");
    }
}
...
void minimal_app_state_handle_input_event(const input_event_t *event)
{
    ...
    switch (event->type) {
        ...
        case INPUT_EVENT_ZIGBEE_LIGHT_ONOFF_SET:
            if (event->value != 0) restore_last("ha_light_on"); else all_off("ha_light_off");
            break;
        ...
        case INPUT_EVENT_ZIGBEE_RS232_SET:
            set_rs232(event->value != 0, "ha_rs232");
            break;
        default:
            break;
    }
