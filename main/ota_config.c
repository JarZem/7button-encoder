...
{
    if (config != NULL) {
        secure_bzero(config, sizeof(*config));
    }
}
...
static bool token_is_valid(const char *token)
{
    for (size_t i = 0; i < OTA_CONFIG_TOKEN_LEN; ++i) {
        ...
        if (!digit && !upper && !lower && !base64url) {
            return false;
        }
    }
...
esp_err_t ota_config_parse_payload(const char *payload, size_t payload_len, ota_config_t *config)
{
    ...
    for (size_t i = 0; i < payload_len; ++i) {
        if (payload[i] == '|') {
            ESP_RETURN_ON_FALSE(separator_count < 4,
                                ESP_ERR_INVALID_ARG, TAG, "invalid payload: too many fields");
    ...
    ESP_GOTO_ON_ERROR(copy_field(payload, 0, separators[0],
                                 parsed.ssid, sizeof(parsed.ssid)),
                      cleanup, TAG, "invalid SSID");
    ESP_GOTO_ON_ERROR(copy_field(payload, separators[0] + 1, separators[1],
                                 parsed.password, sizeof(parsed.password)),
                      cleanup, TAG, "invalid password");
    ESP_GOTO_ON_ERROR(copy_field(payload, separators[1] + 1, separators[2],
                                 parsed.host, sizeof(parsed.host)),
                      cleanup, TAG, "invalid host");
    ESP_GOTO_ON_ERROR(copy_field(payload, separators[2] + 1, separators[3],
                                 parsed.code, sizeof(parsed.code)),
                      cleanup, TAG, "invalid firmware code");
    ESP_GOTO_ON_ERROR(copy_field(payload, separators[3] + 1, payload_len,
                                 parsed.token, sizeof(parsed.token)),
                      cleanup, TAG, "invalid token");

    ESP_GOTO_ON_FALSE(strlen(parsed.code) == OTA_CONFIG_CODE_LEN && code_is_valid(parsed.code),
                      ESP_ERR_INVALID_ARG, cleanup, TAG, "invalid payload: firmware code");
    ESP_GOTO_ON_FALSE(strlen(parsed.token) == OTA_CONFIG_TOKEN_LEN && token_is_valid(parsed.token),
                      ESP_ERR_INVALID_ARG, cleanup, TAG, "invalid payload: token");

    *config = parsed;

cleanup:
    if (ret != ESP_OK) {
        ota_config_clear(&parsed);
    }
    return ret;
}
