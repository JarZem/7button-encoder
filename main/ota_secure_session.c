                                        ...
                                        uint8_t *out, size_t out_len)
{
    ...
    for (size_t i = 0; i < padded_len; ++i) {
        if (padded[i] == '-') padded[i] = '+';
                   ...
                   (padded[i] >= '0' && padded[i] <= '9'))) return ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t device_id(char out[DEVICE_ID_MAX_LEN])
{
...
{
    state_nvs_t rec = {
        .magic = OTA_SEC_STATE_MAGIC,
        .state = (uint8_t)s_state,
        .counter = s_counter,
    };
    memcpy(rec.random, s_random, sizeof(rec.random));
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(OTA_SEC_NAMESPACE, NVS_READWRITE, &h), TAG, "open state NVS failed");
    esp_err_t err = nvs_set_blob(h, OTA_SEC_STATE_KEY, &rec, sizeof(rec));
...
static esp_err_t persist_provisioning(const ota_secure_provisioning_t *config)
{
    ...
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(OTA_SEC_NAMESPACE, NVS_READWRITE, &h), TAG, "open provisioning NVS failed");
    esp_err_t err = nvs_set_blob(h, OTA_SEC_PROVISION_KEY, &rec, sizeof(rec));
    if (err == ESP_OK) err = nvs_commit(h);
    nvs_close(h);
    memset(&rec, 0, sizeof(rec));
    return err;
                                    ...
                                    uint8_t out[32])
{
    ...
    ESP_RETURN_ON_ERROR(device_id(id), TAG, "device id unavailable");
    uint8_t material[128];
    size_t pos = 0;
    const size_t dlen = strlen(KDF_DOMAIN);
    const size_t ilen = strlen(id);
    memcpy(material + pos, KDF_DOMAIN, dlen); pos += dlen;
    memcpy(material + pos, id, ilen); pos += ilen;
    put_u64_be(material + pos, counter); pos += 8;
...
static esp_err_t build_challenge_canonical(uint64_t counter,
                                           const uint8_t random[OTA_SECURE_RANDOM_LEN],
                                           uint8_t *out, size_t out_size, size_t *out_len)
{
    char id[DEVICE_ID_MAX_LEN];
    ESP_RETURN_ON_ERROR(device_id(id), TAG, "device id unavailable");
    int n = snprintf((char *)out, out_size, "A|%s|%llu|", id, (unsigned long long)counter);
    if (n <= 0 || (size_t)n + OTA_SECURE_RANDOM_LEN > out_size) return ESP_ERR_INVALID_SIZE;
    memcpy(out + n, random, OTA_SECURE_RANDOM_LEN);
    *out_len = (size_t)n + OTA_SECURE_RANDOM_LEN;
    return ESP_OK;
}

static esp_err_t build_response_canonical(uint64_t counter,
                                          const uint8_t random[OTA_SECURE_RANDOM_LEN],
                                          uint8_t *out, size_t out_size, size_t *out_len)
{
    char id[DEVICE_ID_MAX_LEN];
    ESP_RETURN_ON_ERROR(device_id(id), TAG, "device id unavailable");
    int n = snprintf((char *)out, out_size, "R|%s|%llu|", id, (unsigned long long)counter);
    if (n <= 0 || (size_t)n + OTA_SECURE_RANDOM_LEN + 3 > out_size) return ESP_ERR_INVALID_SIZE;
    memcpy(out + n, random, OTA_SECURE_RANDOM_LEN);
    memcpy(out + n + OTA_SECURE_RANDOM_LEN, "|OK", 3);
    *out_len = (size_t)n + OTA_SECURE_RANDOM_LEN + 3;
    return ESP_OK;
}

static esp_err_t build_nonce_aad(uint8_t nonce[OTA_SEC_GCM_NONCE_LEN],
                                 uint8_t *aad, size_t aad_size, size_t *aad_len)
{
    ...
    uint8_t material[128];
    size_t pos = 0;
    const size_t ndlen = strlen(NONCE_DOMAIN);
    const size_t ilen = strlen(id);
    memcpy(material + pos, NONCE_DOMAIN, ndlen); pos += ndlen;
    memcpy(material + pos, id, ilen); pos += ilen;
    put_u64_be(material + pos, s_counter); pos += 8;
    memcpy(material + pos, s_random, sizeof(s_random)); pos += sizeof(s_random);
    uint8_t hash[32];
    ESP_RETURN_ON_ERROR(hmac_sha256(s_session_key, sizeof(s_session_key), material, pos, hash), TAG, "nonce KDF failed");
    memcpy(nonce, hash, OTA_SEC_GCM_NONCE_LEN);

    int n = snprintf((char *)aad, aad_size, "P|%s|%llu|", id, (unsigned long long)s_counter);
    if (n <= 0 || (size_t)n + sizeof(s_random) > aad_size) return ESP_ERR_INVALID_SIZE;
    memcpy(aad + n, s_random, sizeof(s_random));
    *aad_len = (size_t)n + sizeof(s_random);
    memset(material, 0, sizeof(material));
                                           ...
                                           ota_secure_provisioning_t *config)
{
    ...
    memcpy(config->password, plain + pos, pass_len); pos += pass_len;
    if (host_type == 1) {
        ...
        memcpy(config->ota_host, plain + pos, host_len); pos += host_len;
    } else return ESP_ERR_INVALID_ARG;
    config->ota_port = get_u16_be(plain + pos);
    return config->ota_port == 0 ? ESP_ERR_INVALID_ARG : ESP_OK;
}
...
esp_err_t ota_secure_session_init(void)
{
    ...
    ota_secure_provisioning_t cfg;
    if (ota_secure_session_load_provisioning(&cfg) == ESP_OK) s_state = OTA_SEC_STATE_PROVISIONED;
    memset(&cfg, 0, sizeof(cfg));
    ESP_LOGI(TAG, "state=%s", ota_secure_session_state_name());
    return ESP_OK;
}

esp_err_t ota_secure_session_begin_hello(uint64_t counter)
{
    if (s_state != OTA_SEC_STATE_IDLE) {
        ESP_LOGW(TAG, "HELLO state transition rejected current=%s", ota_secure_session_state_name());
        return ESP_ERR_INVALID_STATE;
    }
    if (counter == 0) return ESP_ERR_INVALID_ARG;
    s_counter = counter;
    memset(s_random, 0, sizeof(s_random));
    memset(s_session_key, 0, sizeof(s_session_key));
    s_state = OTA_SEC_STATE_WAIT_CHALLENGE;
    ESP_RETURN_ON_ERROR(persist_state(), TAG, "persist WAIT_CHALLENGE failed");
    ESP_LOGI(TAG, "state IDLE -> WAIT_CHALLENGE counter=%llu", (unsigned long long)counter);
    return ESP_OK;
}
...
{
    if (payload == NULL || response_out == NULL) return ESP_ERR_INVALID_ARG;
    response_out[0] = '\0';
    if (s_state != OTA_SEC_STATE_WAIT_CHALLENGE) {
        ESP_LOGW(TAG, "A replay/out-of-order dropped state=%s", ota_secure_session_state_name());
        return ESP_ERR_INVALID_STATE;
    }

    char work[OTA_SECURE_PROVISION_MAX_WIRE_LEN + 1];
    const size_t len = strlen(payload);
    if (len > OTA_SECURE_PROVISION_MAX_WIRE_LEN) return ESP_ERR_INVALID_SIZE;
    memcpy(work, payload, len + 1);
    char *save = NULL;
    char *kind = strtok_r(work, "|", &save);
    char *random_text = strtok_r(NULL, "|", &save);
    char *sig_text = strtok_r(NULL, "|", &save);
    ...
    ESP_RETURN_ON_ERROR(build_challenge_canonical(s_counter, random, canonical, sizeof(canonical), &canonical_len), TAG, "challenge canonical failed");
...
esp_err_t ota_secure_session_accept_provisioning(const char *payload)
{
    ...
    if (s_state != OTA_SEC_STATE_WAIT_PROVISIONING) {
        ESP_LOGW(TAG, "P replay/out-of-order dropped state=%s", ota_secure_session_state_name());
        return ESP_ERR_INVALID_STATE;
    }
    ...
    size_t binary_len = 0;
    ESP_RETURN_ON_ERROR(base64url_decode_variable(payload + 2, binary, sizeof(binary), &binary_len), TAG, "P base64 invalid");
    if (binary_len <= OTA_SEC_GCM_TAG_LEN) return ESP_ERR_INVALID_SIZE;
    const size_t cipher_len = binary_len - OTA_SEC_GCM_TAG_LEN;
    if (cipher_len > OTA_SEC_MAX_PLAINTEXT) return ESP_ERR_INVALID_SIZE;

    uint8_t nonce[OTA_SEC_GCM_NONCE_LEN];
    uint8_t aad[128];
    size_t aad_len = 0;
    ESP_RETURN_ON_ERROR(build_nonce_aad(nonce, aad, sizeof(aad), &aad_len), TAG, "P crypto context failed");

    uint8_t plain[OTA_SEC_MAX_PLAINTEXT];
    mbedtls_gcm_context gcm;
    mbedtls_gcm_init(&gcm);
    int ret = mbedtls_gcm_setkey(&gcm, MBEDTLS_CIPHER_ID_AES, s_session_key, 256);
    if (ret == 0) {
        ret = mbedtls_gcm_auth_decrypt(&gcm, cipher_len,
                                       nonce, sizeof(nonce), aad, aad_len,
                                       binary + cipher_len, OTA_SEC_GCM_TAG_LEN,
                                       binary, plain);
    }
    mbedtls_gcm_free(&gcm);
    if (ret != 0) {
        ESP_LOGE(TAG, "P rejected: AES-GCM authentication failed");
        return ESP_ERR_INVALID_CRC;
    }

    ota_secure_provisioning_t cfg;
    esp_err_t err = parse_provision_plaintext(plain, cipher_len, &cfg);
    if (err == ESP_OK) err = persist_provisioning(&cfg);
    if (err == ESP_OK) {
        s_state = OTA_SEC_STATE_PROVISIONED;
        err = persist_state();
    }
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "P verified+stored; state WAIT_PROVISIONING -> PROVISIONED ssid=%s ota=%s:%u security=%u channel=%u password_len=%u",
                 cfg.ssid, cfg.ota_host, (unsigned)cfg.ota_port,
                 (unsigned)cfg.wifi_security, (unsigned)cfg.wifi_channel,
...
{
    if (s_state == OTA_SEC_STATE_PROVISIONED) return;
    ESP_LOGW(TAG, "session timeout/reset state=%s -> IDLE", ota_secure_session_state_name());
    s_state = OTA_SEC_STATE_IDLE;
    s_counter = 0;
    memset(s_random, 0, sizeof(s_random));
    memset(s_session_key, 0, sizeof(s_session_key));
...
{
    switch (s_state) {
        case OTA_SEC_STATE_IDLE: return "IDLE";
        case OTA_SEC_STATE_WAIT_CHALLENGE: return "WAIT_CHALLENGE";
        case OTA_SEC_STATE_WAIT_PROVISIONING: return "WAIT_PROVISIONING";
        case OTA_SEC_STATE_PROVISIONED: return "PROVISIONED";
...
esp_err_t ota_secure_session_load_provisioning(ota_secure_provisioning_t *out)
{
    ...
    err = nvs_get_blob(h, OTA_SEC_PROVISION_KEY, &rec, &size);
    nvs_close(h);
    if (err != ESP_OK || size != sizeof(rec) || rec.magic != OTA_SEC_PROVISION_MAGIC) {
        memset(&rec, 0, sizeof(rec));
