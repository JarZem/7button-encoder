...
static void timer_cb(void *arg)
{
    ...
    if (fatal) {
        set_pixel((tick % 2) == 0 ? COLOR_RED : COLOR_OFF);
        return;
    }
    ...
    if (failed) {
        set_pixel((tick % 6) < 3 ? COLOR_RED : COLOR_OFF);
        return;
    }

    if (pairing) {
        set_pixel((tick % 4) < 2 ? COLOR_BLUE : COLOR_OFF);
        return;
    }

    if (!joined) {
        set_pixel((tick % 6) < 3 ? COLOR_PURPLE : COLOR_OFF);
        return;
    }
...
void status_led_init(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Status LED timer failed: %s", esp_err_to_name(err));
        return;
    }
...
{
    start_pulse(PULSE_LOCAL_GREEN);
}

void status_led_indicate_ha_command(void)
{
    start_pulse(PULSE_HA_COMMAND_YELLOW);
}

void status_led_indicate_ha_publish(void)
{
    start_pulse(PULSE_HA_PUBLISH_GREEN);
}
