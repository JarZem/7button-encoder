...
void zigbee_ota_control_set_enabled(bool enabled)
{
    ...
    if (!enabled && ota_secure_session_is_provisioned()) {
        ...
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "could not snapshot provisioning context for OTA CHECK: %s", esp_err_to_name(err));
        }
    }
}
...
static void initial_report_task(void *arg)
{
    ...
    for (unsigned attempt = 1; attempt <= INITIAL_REPORT_REPEAT_COUNT; ++attempt) {
        ...
        ESP_LOGI(TAG, "initial HA state uplink attempt=%u EnableOTA=0 Status=0x00 result=%s",
        ...
        if (state != previous) {
            switch (state) {
                ...
                case OTA_STATE_DOWNLOADING:
                    if (s_status != ZIGBEE_OTA_STATUS_FW_UPDATE_STARTED)
...
esp_err_t zigbee_ota_control_add_endpoint(esp_zb_ep_list_t *ep_list)
{
    ...
    ESP_RETURN_ON_ERROR(esp_zb_cluster_add_manufacturer_attr(enable_cluster, ZIGBEE_OTA_ENABLE_CLUSTER_ID,
                            ZIGBEE_OTA_ENABLE_ATTR_ID, ZIGBEE_OTA_CONTROL_MANUFACTURER_CODE,
                            ESP_ZB_ZCL_ATTR_TYPE_BOOL, enable_access, &s_enable_ota),
    ...
    ESP_RETURN_ON_ERROR(esp_zb_cluster_add_manufacturer_attr(status_cluster, ZIGBEE_OTA_STATUS_CLUSTER_ID,
                            ZIGBEE_OTA_STATUS_ATTR_ID, ZIGBEE_OTA_CONTROL_MANUFACTURER_CODE,
                            ESP_ZB_ZCL_ATTR_TYPE_U8, status_access, &s_status),
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGI(TAG, "endpoint=%u EnableOTA cluster=0x%04x default=0; Status cluster=0x%04x default=0",
        ...
        if (!s_initial_report_started) {
            s_initial_report_started = xTaskCreate(initial_report_task, "ota_initial", 3072, NULL, 4, NULL) == pdPASS;
            if (!s_initial_report_started) ESP_LOGW(TAG, "initial OTA state report task creation failed");
        }
    }
    return err;
}
...
bool zigbee_ota_control_handle_set_attr(const esp_zb_zcl_set_attr_value_message_t *message)
{
    if (message == NULL || message->info.status != ESP_ZB_ZCL_STATUS_SUCCESS ||
        ...
        message->attribute.id != ZIGBEE_OTA_ENABLE_ATTR_ID) return false;
    ...
    if (xTaskCreate(apply_enable_task, "ota_enable", 3072, (void *)(uintptr_t)(s_enable_ota ? 1U : 0U), 5, NULL) != pdPASS)
