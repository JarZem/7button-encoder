...
const endpointNameById=(endpointId)=>{for(const [name,id] of Object.entries(ENDPOINTS)){if(id===endpointId)return name;}return undefined;};
const clampNumber=(value,min,max)=>{const number=Number(value);if(!Number.isFinite(number))return min;return Math.min(max,Math.max(min,Math.round(number)));};
...
const endpointForName=(entity,meta,endpointName)=>{const endpointId=ENDPOINTS[endpointName];return endpointId===undefined?entity:(meta?.device?.getEndpoint(endpointId)??entity);};
