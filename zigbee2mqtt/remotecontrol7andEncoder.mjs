...
const remoteFromOtaCommand={cluster:OTA_CLUSTER_NAME,type:['commandOtaFromDevice'],convert:(model,msg,publish,options,meta)=>{if(msg.endpoint.ID!==OTA_ENDPOINT)return;const v=msg.data?.payload;if(v==null)return;return otaUplinkState(msg,meta,String(v));}};
const remoteFromOtaRaw={cluster:OTA_CLUSTER_NAME,type:['raw'],convert:(model,msg,publish,options,meta)=>{if(msg.endpoint.ID!==OTA_ENDPOINT)return;const raw=Buffer.isBuffer(msg.data)?msg.data:msg.data?.data;if(raw==null)return;const b=Buffer.isBuffer(raw)?raw:Buffer.from(raw);if(b.length<2)return;const direct=b.toString('utf8');if(/^(H|D|R|F|T)\|/.test(direct))return otaUplinkState(msg,meta,direct);if(b.length>=4&&b[2]===OTA_CMD_FROM_DEVICE_ID){const n=b[3];if(n<1||b.length<4+n)return;const payload=b.subarray(4,4+n).toString('utf8');return otaUplinkState(msg,meta,payload);}}};
const remoteFromOtaEnable={cluster:OTA_ENABLE_CLUSTER_NAME,type:['attributeReport','readResponse'],convert:(model,msg)=>{if(msg.endpoint.ID!==OTA_CONTROL_ENDPOINT||msg.data?.[OTA_ENABLE_ATTR_NAME]===undefined)return;return{enable_ota_ota_control:msg.data[OTA_ENABLE_ATTR_NAME]?'ON':'OFF'};}};
const remoteFromOtaStatus={cluster:OTA_STATUS_CLUSTER_NAME,type:['attributeReport','readResponse'],convert:(model,msg)=>{if(msg.endpoint.ID!==OTA_CONTROL_ENDPOINT||msg.data?.[OTA_STATUS_ATTR_NAME]===undefined)return;return{ota_status_ota_control:decodeOtaStatus(msg.data[OTA_STATUS_ATTR_NAME])};}};
