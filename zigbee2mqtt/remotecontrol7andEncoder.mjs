...
const otaUplinkState=(msg,meta,payload)=>{logOtaUplink(msg,meta,payload);const control=decodeOtaControlUplink(payload);return control??{action:payload};};
...
const remoteFromOtaStatus={cluster:OTA_STATUS_CLUSTER_NAME,type:['attributeReport','readResponse'],convert:(model,msg)=>{if(msg.endpoint.ID!==OTA_CONTROL_ENDPOINT||msg.data?.[OTA_STATUS_ATTR_NAME]===undefined)return;return{ota_status:decodeOtaStatus(msg.data[OTA_STATUS_ATTR_NAME])};}};
